...
def draw_efficiency(df, grid_dim, linear_system):
    ...
    gs_np /= gauss_seidel["nr_processes"].values
    dj_np /= damped_jacobi["nr_processes"].values

    gauss_seidel["efficiency"] = gs_np
    damped_jacobi["efficiency"] = dj_np
    ...
    pl_optimal = p.line(
        x=np.arange(1, 65),
        y=np.ones(64),
        legend_label="optimal efficiency",
        line_color="green",
    )
...
def draw_efficiency_static(df, grid_dim, linear_system, solver, colors, title):
    p = figure(
            title=title,
            tools="pan, wheel_zoom, reset, save",
            x_axis_label="Number of processes",
    ...
    for gd, c in zip(grid_dim, colors):
        ...
        solver_np /= solver_data["nr_processes"].values

        solver_data["efficiency"] = solver_np
    ...
    if solver == "gauss-seidel" or linear_system == "nine-point":
        loc = "top_right"
    else:
        loc = "bottom_left"
